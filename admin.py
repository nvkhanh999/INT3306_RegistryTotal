...
async def getAllAdmins(db: Session = Depends(getDatabase), userId : int = Depends(oauth2.getCurrentUser)):
    admins = db.query(models.Admin).all()
    return admins

@router.get('/latest',response_model=schemas.AdminResponse)
async def getLatest(db: Session = Depends(getDatabase), userId : int = Depends(oauth2.getCurrentUser)):
...
async def getAdmin(id: int, db: Session = Depends(getDatabase), userId : int = Depends(oauth2.getCurrentUser)):
...
async def createAdmin(admin: schemas.AdminCreate, db: Session = Depends(getDatabase), userId : int = Depends(oauth2.getCurrentUser)):
...
async def update(id: int, admin: schemas.AdminUpdate, db: Session = Depends(getDatabase), userId : int = Depends(oauth2.getCurrentUser)):
    adminQuery = db.query(models.Admin).filter(models.Admin.admin_id == id)
