...
async def get(db: Session = Depends(getDatabase)):
...
async def get(id: int, db: Session = Depends(getDatabase)):
...
async def create(center: schemas.CenterCreate, db: Session = Depends(getDatabase)):
...
async def update(id: int, center: schemas.CenterUpdate, db: Session = Depends(getDatabase)):
    centerQuery = db.query(models.Center).filter(models.Center.center_id == id)
